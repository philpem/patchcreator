...
`patchcreator export --text paths` supports horizontal direct-content `<text>` plus PatchCreator's generated top/bottom circular arc titles.
...
For top/bottom arc text, the exporter resolves the referenced PatchCreator construction baseline before construction guides are removed, parses the single circular `M … A …` baseline generated by the text component, then places each shaped glyph on the path tangent. `startOffset`, `text-anchor`, parent `dy` baseline shift, letter spacing and `textLength` spacing adjustment are applied before the glyph outlines are written. Clockwise and counterclockwise arcs are both supported.

The current pass deliberately rejects constructs it cannot place faithfully yet, including:

- arbitrary/external or compound `<textPath>` geometry that is not a PatchCreator circular construction baseline;
- `<tspan>` children;
...
Those failures are intentional rather than approximations. A later #55 slice will generalise path-following text beyond the circular PatchCreator arc form.
